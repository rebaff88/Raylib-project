...
// Declare enemy variables
		...
		// Inside your main game loop, add this code before EndDrawing():
		...
		if (!GameOver) {
			...
			for (int i = 0; i < MaxEnemies; i++) {
				if (EnemyActive[i]) {
					...
					if (EnemyPosY[i] + 30 > ShipH && EnemyPosY[i] < ShipH + 40 &&  // Y overlap
						EnemyPosX[i] + 30 > ShipW && EnemyPosX[i] < ShipW + ShipWidth) { // X overlap
			...
			for (int i = 0; i < MaxEnemies; i++) {
				if (EnemyActive[i]) {
					...
					// optional: add more visual detail for enemies
				}
			}
			...
			if (GameOver) {
				...
				// Freeze game or wait for restart
				// You can add a restart key if desired
				if (IsKeyPressed(KEY_ENTER)) {
					// Restart game
					Score = 0;
					GameOver = false;
